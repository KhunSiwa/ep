...
- `/planner` Planner สำหรับค้นหา เพิ่ม แก้ไข ลบ เปลี่ยนสถานะ task และติดตาม progress/checklist
...
- `frontend/src/taskOptions.js` กำหนดสถานะ task และ logic คำนวณ progress
...
```json
{
  ...
  "progress": 50,
  "checklist": [
    { "id": "step-1", "text": "Write launch copy", "done": true },
    { "id": "step-2", "text": "Review with team", "done": false }
  ]
}
```
...
```text
backlog, pending, in_progress, blocked, review, completed
```

ถ้า task มี checklist ระบบจะคำนวณ `progress` จากจำนวนขั้นตอนที่ติ๊กเสร็จแล้วโดยอัตโนมัติ เช่น checklist 4 ขั้นตอน ติ๊กเสร็จ 2 ขั้นตอน จะได้ `progress = 50`
